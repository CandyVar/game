...
def load_image(name, colorkey=None):
    ...
    if not os.path.isfile(fullname):
        print(f"File with the image '{fullname}' not found")
        sys.exit()
...
def start_screen():
    intro_text = ["Title Screen", "",
                  "Game Rules",
                  "If there are multiple lines in the rules,",
                  "they need to be displayed line by line"]
...
def fade_out_and_load_new_world(screen, clock, new_map_filename):
    ...
    fade_steps = 50  # Number of steps to reach full transparency
    ...
    # Load and generate a new world based on the provided map filename
    new_player, level_x, level_y = generate_level(load_level(new_map_filename))

    # Reset the camera and sprite groups
